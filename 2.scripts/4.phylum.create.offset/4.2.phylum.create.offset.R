...
read.table(file="2.scripts/4.phylum.create.offset/final.phylum.txt", sep="\t", header=TRUE, check.names = FALSE) -> emp.orig_all # this is the data for the offset-calculation-file-table
load(file="3.results/phylum.create.data.output/factors.RData")
...
save(emp.sum.final, file = paste(dir, "emp.sum.final.phylum", ".RData", sep = ""))
rm(dir)
